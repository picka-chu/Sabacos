...
function buildInitData(opts: { authDate?: number; signature?: string; queryId?: string | null } = {}): string {
  ...
  // Keyboard-button / menu-button / direct-link launches carry no query_id.
  if (opts.queryId === undefined) pairs.query_id = "AAHd3SAAAAA";
  else if (opts.queryId !== null) pairs.query_id = opts.queryId;
...
describe("initData validation", () => {
  ...
  it("accepts keyboard/menu launches without query_id", async () => {
    const initData = buildInitData({ queryId: null });
    expect(initData).not.toContain("query_id");
    expect(await verifyInitDataHash(initData, BOT_TOKEN)).toBe(true);
    const result = await validateInitData(initData, BOT_TOKEN);
    expect(result.valid).toBe(true);
    expect(result.payload?.userId).toBe(987654321);
    expect(result.payload?.queryId).toBeUndefined();
  });
