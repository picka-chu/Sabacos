...
const ROLE_BADGE_CLASS: Record<ProfileRole, string> = {
  admin: "badge-danger",
  staff: "badge-info",
...
export function UsersPage() {
  ...
  async function fetchUsers() {
    ...
    try {
      ...
      const res = await api.get<{ items: Profile[]; total: number }>(
  ...
  async function handleInvite() {
    ...
    try {
      await api.post(
        "/admin/users/invite",
        { telegramId: Number(inviteTelegramId), role: inviteRole },
        token ?? undefined,
      );
      setShowInvite(false);
      setInviteTelegramId("");
    ...
    <div>
        ...
        <div className="input-row" style={{ gridTemplateColumns: "1fr 200px" }}>
          ...
          <div className="field" style={{ marginBottom: 0 }}>
            <select
              ...
              value={roleFilter}
              onChange={(e) => {
                setRoleFilter(e.target.value as ProfileRole | "");
                setPage(1);
              }}
            >
      ...
      <div className="card">
        {loading ? (
          ...
          <div className="empty">
            <Users size={40} strokeWidth={1.25} style={{ color: "var(--muted)", marginBottom: 12 }} />
            <h3>No users found</h3>
            <p style={{ color: "var(--muted)" }}>Invite users by their Telegram ID to get started.</p>
          </div>
        ) : (
          <>
                  ...
                  {users.map((u) => (
                    <tr key={u.id}>
                      ...
                      <td data-label="Telegram ID">
                        <span className="muted">{u.telegramId ?? "—"}</span>
                      </td>
                      <td data-label="Role">
                        {editingId === u.id ? (
                          <select
                          ...
                          <span
                            className={`badge ${ROLE_BADGE_CLASS[u.role]}`}
            ...
            {totalPages > 1 && (
              <div className="spread" style={{ marginTop: 16, paddingTop: 14, borderTop: "1px solid var(--border)" }}>
                <span className="muted" style={{ fontSize: 13 }}>
                  {total} user(s) · Page {page} of {totalPages}
                </span>
                <div className="row">
                  <button
                    className="btn btn-outline btn-sm"
      ...
      {showInvite && (
        <div className="modal-backdrop" onClick={() => setShowInvite(false)}>
          <div className="card modal-card" onClick={(e) => e.stopPropagation()}>
            <h2 className="page-title" style={{ fontSize: 18, marginBottom: 16 }}>
              Invite User
            </h2>
            <div className="field">
              ...
              <span className="muted" style={{ fontSize: 12 }}>
                Ask the user to send /start to the bot, then check their Telegram ID.
              </span>
            </div>
            <div className="field">
              <label>Role</label>
            ...
            <div className="row" style={{ justifyContent: "flex-end", marginTop: 16 }}>
              <button className="btn btn-outline" onClick={() => setShowInvite(false)}>
                Cancel
              </button>
              <button
                className="btn btn-primary"
                onClick={handleInvite}
                disabled={inviteLoading || !inviteTelegramId.trim()}
              >
