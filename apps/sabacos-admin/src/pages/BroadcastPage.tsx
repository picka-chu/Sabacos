...
export function BroadcastPage() {
  ...
  useEffect(() => {
    if (!token) return;
    api
      .get<{ count: number }>("/admin/broadcast/audience", token)
        ...
        <div className="field">
          ...
          {imageUrl && (
            <div style={{ marginBottom: 8 }}>
              <img src={imageUrl} alt="" style={{ maxWidth: 180, borderRadius: 10 }} />
            </div>
          )}
        ...
        <div className="input-row">
          ...
          <div className="field">
            ...
            <input
              ...
              placeholder="https://sabacos-web.onrender.com/product/…"
            />
          </div>
        </div>

        <button className="btn btn-primary" disabled={sending || !text.trim()} onClick={send}>
          {sending ? <Megaphone size={16} /> : <Send size={16} />}
          {sending ? "Sending…" : "Send broadcast"}
        </button>
        <p className="muted" style={{ margin: "10px 0 0", fontSize: 12 }}>
          Delivery takes about 1 second per 20 users. Image messages show the photo with your
          message as caption; buttons open any link.
        </p>
      </div>
    </>
  );
